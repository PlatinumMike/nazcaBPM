...
void Engine::run() const {
    ...
    const double x_min = -0.5 * inputs.domain_len_x;
    const double x_max = 0.5 * inputs.domain_len_x;
    const double y_min = -0.5 * inputs.domain_len_y;
    const double y_max = 0.5 * inputs.domain_len_y;
    const double z_min = 0.0;
    const double z_max = inputs.domain_len_z;

    // define geometry
    const Geometry geometry(inputs.shapes, inputs.background_index);
    // set perfectly matched layers
    const PML pmlx(inputs.pml_thickness, inputs.pml_strength, x_min, x_max); //adds a PML on both sides
    const PML pmly(inputs.pml_thickness, inputs.pml_strength, y_min, y_max);
    // setup mode source
